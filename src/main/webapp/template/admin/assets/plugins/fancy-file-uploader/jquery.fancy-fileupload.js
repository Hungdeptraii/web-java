	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			...
			var StartUpload = function(e) {
				...
				inforow.find('button.ff_fileupload_remove_file').attr('aria-label', Translate('Hủy tải lên và xóa khỏi danh sách'));
			...
			var RemoveFile = function(e) {
				...
				{
					if (!confirm(Translate('Tập tin này hiện đang được tải lên.\n\nDừng tải lên và xóa file khỏi danh sách?')))  return;
			...
			if (haspreview)
			{
				inforow.find('.ff_fileupload_preview_image').addClass('ff_fileupload_preview_image_has_preview').attr('aria-label', Translate('Xem')).click(function(e) {
					e.preventDefault();
			...
			{
				inforow.find('.ff_fileupload_preview_image').prop('disabled', true).attr('aria-label', Translate('Không có bản xem trước nào')).click(function(e) {
					e.preventDefault();
				});
			}
			...
			if (settings.accept)
			{
				...
				if (!found)  data.ff_info.errors.push(Translate('Phần mở rộng tập tin không hợp lệ.'));
			}

			if (settings.maxfilesize > -1 && data.files[0].size > settings.maxfilesize)  data.ff_info.errors.push(FormatStr(Translate('Tệp quá lớn. Kích thước tệp tối đa là {0}.'), GetDisplayFilesize(settings.maxfilesize, settings.adjustprecision, settings.displayunits)));
			...
			{
				inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Tải lên')).click(StartUpload));
				inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Tải lên')).click(StartUpload));

				inforow.addClass('ff_fileupload_queued');
			}

			inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('Xóa khỏi danh sách')).click(RemoveFile));
			inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('Xóa khỏi danh sách')).click(RemoveFile));
		...
		var UploadFailed = function(e, data) {
			...
			{
				data.ff_info.fileinfo.text(FormatStr(Translate('{0} | Lỗi mạng, sẽ thử lại sau giây lát... ({1})'), data.ff_info.displayfilesize, data.errorThrown));
			...
			{
				// Set the error info.
				if (data.errorThrown === 'abort')  data.ff_info.errors.push(Translate('Quá trình tải lên đã bị hủy.'));
				else if (data.errorThrown === 'failed_with_msg')  data.ff_info.errors.push(FormatStr(Translate('Tải lên không thành công.  {0} ({1})'), EscapeHTML(data.result.error), EscapeHTML(data.result.errorcode)));
				else  data.ff_info.errors.push(Translate('Tải lên không thành công.'));
				...
				// Alter remove buttons.
				data.ff_info.inforow.find('button.ff_fileupload_remove_file').attr('aria-label', Translate('Xóa khỏi danh sách'));
			}
		};
		...
		var UploadDone = function(e, data) {
			...
			{
				if (typeof(data.result.error) !== 'string')  data.result.error = Translate('Máy chủ cho biết quá trình tải lên không thành công. Không có thông tin bổ sung có sẵn.');
			...
			{
				// Set the status.
				data.ff_info.inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize + ' | ' + Translate('Tải lên đã hoàn tất'));
