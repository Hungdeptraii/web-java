...
<c:set var="pageTitle" value="Đơn bị huỷ" scope="request" />
...
<main class="main-wrapper">
    <div class="main-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <h5 class="pe-3">Đơn hàng đã bị huỷ</h5>
        </div>
                            ...
                            <c:forEach var="customer_order" items="${customer_orders}">
                                <tr id="order-row-${customer_order.id}">
                                    ...
                                    <td class="text-center" width="15%">
                                        ...
                                        <button type="button" class="btn btn-danger btn-delete-order" data-order-id="${customer_order.id}" data-bs-toggle="modal" data-bs-target="#delete_order_${customer_order.id}" title="Xoá đơn">
                                            <i class="material-icons-outlined">delete</i>
                                        </button>
                                        <!-- Modal xác nhận xoá đơn -->
                                        <div class="modal fade" id="delete_order_${customer_order.id}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">Xác nhận xoá đơn</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <h5>Bạn có chắc chắn muốn xoá đơn <b>${customer_order.code}</b> không?</h5>
                                                        <div class="modal-footer d-flex justify-content-center">
                                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Không</button>
                                                            <button type="button" class="btn btn-danger btn-delete-order-confirmed" data-order-id="${customer_order.id}" data-bs-dismiss="modal">Đồng ý</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="modal fade text-start" id="payment_${customer_order.id}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-scrollable">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="mb-3">
                                                            <label class="form-label"><b>Lý do huỷ đơn:</b> ${customer_order.reason}</label>
                                                        </div>
                                                        <div class="mb-3">
...
<script>
    $(document).ready(function () {
        ...
        // Xử lý xoá đơn bị huỷ
        $('.btn-delete-order-confirmed').click(function() {
            var orderId = $(this).data('order-id');
            $.post('/admin/delete-order-cancelled', {orderId: orderId}, function(res) {
                if (res === 'success') {
                    toastr.success('Xoá đơn hàng thành công!', 'Thông báo');
